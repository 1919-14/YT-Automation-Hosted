...
def chat(messages, temperature=0.9, max_retries=3, **kwargs):
...
def chat_json(messages, temperature=0.7, max_retries=3, **kwargs):
    ...
    last_err = None
    for attempt in range(max_retries):
        raw = chat(messages, temperature=temperature, max_retries=1, **kwargs)
        cleaned = raw.strip()
        ...
        try:
            ...
            print(f"[llm_client] raw output was: {raw[:300]}")
